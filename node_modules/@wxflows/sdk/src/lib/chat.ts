...
export async function fetchTools({
  ...
  const data = await request<{tc_tools: WxflowsTool[]}>({
    url: endpoint,
    document: query,
...
export function createChatSystemPrompt(tools: WxflowsTool[]): string {
...
export async function fetchOpenAIChatContent({
  endpoint,
  apikey,
  messages,
  tools,
  ...
  const fetchSingleOpenAIChatContent = async () => {
    ...
    const data = await request<{
      openAI_ChatContent: OpenAIChatContent[] | null
    }>({
      url: endpoint,
      document: query,
      variables,
  ...
  if (
    ...
    if (result[0].message.tool_calls) {
      ...
      messages.push(...(toolCallMessages.filter(Boolean) as ChatMessage[]))
    }
